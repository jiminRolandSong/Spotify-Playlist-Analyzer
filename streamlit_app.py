...
# ════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown("How data flows from Spotify to your dashboard.")
    ...
    ```
    ┌─────────────────┐
    │   Spotify API   │  ← track metadata, artist genres, popularity
    └────────┬────────┘
             │  spotipy (Python)
             ▼
    ┌─────────────────┐
    │  Extract/       │  extract.py  — fetch raw tracks
    │  Transform      │  transform.py — clean dates, durations, nulls
    └────────┬────────┘
             │  SQLAlchemy + psycopg2
             ▼
    ┌─────────────────────────────┐
    │  PostgreSQL (Supabase)      │
    │  table: playlist_tracks     │  ← raw, per-user data
    └────────┬────────────────────┘
             │  dbt run
             ▼
    ┌─────────────────────────────┐
    │  dbt Staging Layer          │
    │  model: stg_tracks          │  ← type casting, null filtering,
    │                             │    column renaming
    └────────┬────────────────────┘
             │  dbt run
             ▼
    ┌─────────────────────────────┐
    │  dbt Mart Layer             │
    │  model: mart_track_stats    │  ← album-level aggregates:
    │                             │    avg popularity, track count,
    │                             │    avg duration per album
    └────────┬────────────────────┘
             │  SQL query
             ▼
    ┌─────────────────┐
    │  Streamlit App  │  ← this page shows the mart output below
    └─────────────────┘
    ```
    """)
    ...
    with col1:
        st.markdown("**Extract & Transform**")
        st.markdown("""
- `scripts/extract.py` calls Spotify API
- Fetches tracks, artist genres, album metadata
- `scripts/transform.py` cleans the data:
  - Parses release dates
  - Converts duration ms → sec
  - Fills null popularity with 0
        """)
    with col2:
        st.markdown("**Load (PostgreSQL)**")
        st.markdown("""
- `scripts/load.py` writes to Supabase
- Table: `playlist_tracks`
- UPSERT logic — no duplicates
- Stores per-user data with `user_id`
- JSONB columns for arrays (artists, genres)
        """)
    with col3:
        st.markdown("**dbt Transformations**")
        st.markdown("""
- `stg_tracks` — staging model:
  - Selects clean columns
  - Casts types, filters nulls
- `mart_track_stats` — mart model:
  - Groups by album + release year
  - Computes avg/max/min popularity
  - Computes avg duration
        """)

    st.markdown("---")

    # ── Live mart output ─────────────────────────────────────────────────────
    st.subheader("Live Output: mart_track_stats")
    st.caption("Aggregated results from dbt — built from your analyzed playlists. Updates automatically after each analysis.")
    ...
    try:
        ...
        with col_b:
            ...
            if "release_year" in mart_df.columns and "avg_duration_sec" in mart_df.columns:
                year_dur = mart_df.dropna(subset=["release_year"])
        ...
        st.subheader("Full mart_track_stats Table")
